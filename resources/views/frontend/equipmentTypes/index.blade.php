        ...
        <div class="col-md-12">
            @can('equipment_type_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.equipment-types.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $equipmentType->id }}">
                                        ...
                                        <td>
                                            @can('equipment_type_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.equipment-types.show', $equipmentType->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('equipment_type_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.equipment-types.edit', $equipmentType->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('equipment_type_delete')
                                                <form action="{{ route('frontend.equipment-types.destroy', $equipmentType->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('equipment_type_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.equipment-types.massDestroy') }}",
