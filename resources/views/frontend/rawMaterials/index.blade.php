        ...
        <div class="col-md-12">
            @can('raw_material_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.raw-materials.create') }}">
                            {{ trans('global.add') }} {{ trans('cruds.rawMaterial.title_singular') }}
                        </a>
                        <button class="btn btn-warning" data-toggle="modal" data-target="#csvImportModal">
                            {{ trans('global.app_csvImport') }}
                        </button>
                        @include('csvImport.modal', ['model' => 'RawMaterial', 'route' => 'admin.raw-materials.parseCsvImport'])
                    </div>
                </div>
            @endcan
            <div class="card">
                                    ...
                                    <tr data-entry-id="{{ $rawMaterial->id }}">
                                        ...
                                        <td>
                                            @can('raw_material_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.raw-materials.show', $rawMaterial->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('raw_material_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.raw-materials.edit', $rawMaterial->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('raw_material_delete')
                                                <form action="{{ route('frontend.raw-materials.destroy', $rawMaterial->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('raw_material_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.raw-materials.massDestroy') }}",
