        ...
        <div class="col-md-12">
            @can('inspection_status_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.inspection-statuses.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $inspectionStatus->id }}">
                                        ...
                                        <td>
                                            @can('inspection_status_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.inspection-statuses.show', $inspectionStatus->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('inspection_status_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.inspection-statuses.edit', $inspectionStatus->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('inspection_status_delete')
                                                <form action="{{ route('frontend.inspection-statuses.destroy', $inspectionStatus->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('inspection_status_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.inspection-statuses.massDestroy') }}",
