        ...
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-header">
                    {{ trans('global.edit') }} {{ trans('cruds.afaStaff.title_singular') }}
                </div>

                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.afa-staffs.update", [$afaStaff->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            <label for="full_name">{{ trans('cruds.afaStaff.fields.full_name') }}</label>
                            <input class="form-control" type="text" name="full_name" id="full_name" value="{{ old('full_name', $afaStaff->full_name) }}">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="user_id" id="user_id">
                                @foreach($users as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $afaStaff->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('user'))
                                <div class="invalid-feedback">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="staff_level_id" id="staff_level_id">
                                @foreach($staff_levels as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('staff_level_id') ? old('staff_level_id') : $afaStaff->staff_level->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="reports_to_id" id="reports_to_id">
                                @foreach($reports_tos as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('reports_to_id') ? old('reports_to_id') : $afaStaff->reports_to->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control date" type="text" name="date_started" id="date_started" value="{{ old('date_started', $afaStaff->date_started) }}">
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control date" type="text" name="date_finished" id="date_finished" value="{{ old('date_finished', $afaStaff->date_finished) }}">
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="staff_bio" id="staff_bio">{{ old('staff_bio', $afaStaff->staff_bio) }}</textarea>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="department_id" id="department_id" required>
                                @foreach($departments as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('department_id') ? old('department_id') : $afaStaff->department->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
