                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.documents.store") }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="related_orders[]" id="related_orders" multiple>
                                @foreach($related_orders as $id => $related_orders)
                                    <option value="{{ $id }}" {{ in_array($id, old('related_orders', [])) ? 'selected' : '' }}>{{ $related_orders }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="related_products[]" id="related_products" multiple>
                                @foreach($related_products as $id => $related_products)
                                    <option value="{{ $id }}" {{ in_array($id, old('related_products', [])) ? 'selected' : '' }}>{{ $related_products }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="related_skus[]" id="related_skus" multiple>
                                @foreach($related_skus as $id => $related_sku)
                                    <option value="{{ $id }}" {{ in_array($id, old('related_skus', [])) ? 'selected' : '' }}>{{ $related_sku }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="related_users[]" id="related_users" multiple>
                                @foreach($related_users as $id => $related_users)
                                    <option value="{{ $id }}" {{ in_array($id, old('related_users', [])) ? 'selected' : '' }}>{{ $related_users }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="related_companies[]" id="related_companies" multiple>
                                @foreach($related_companies as $id => $related_companies)
                                    <option value="{{ $id }}" {{ in_array($id, old('related_companies', [])) ? 'selected' : '' }}>{{ $related_companies }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="related_contacts[]" id="related_contacts" multiple>
                                @foreach($related_contacts as $id => $related_contacts)
                                    <option value="{{ $id }}" {{ in_array($id, old('related_contacts', [])) ? 'selected' : '' }}>{{ $related_contacts }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="file_type_id" id="file_type_id">
                                @foreach($file_types as $id => $entry)
                                    <option value="{{ $id }}" {{ old('file_type_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="authorised_user_types[]" id="authorised_user_types" multiple>
                                @foreach($authorised_user_types as $id => $authorised_user_types)
                                    <option value="{{ $id }}" {{ in_array($id, old('authorised_user_types', [])) ? 'selected' : '' }}>{{ $authorised_user_types }}</option>
                                @endforeach
                            </select>
