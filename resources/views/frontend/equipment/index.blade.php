        ...
        <div class="col-md-12">
            @can('equipment_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.equipment.create') }}">
                            {{ trans('global.add') }} {{ trans('cruds.equipment.title_singular') }}
                        </a>
                        <button class="btn btn-warning" data-toggle="modal" data-target="#csvImportModal">
                            {{ trans('global.app_csvImport') }}
                        </button>
                        @include('csvImport.modal', ['model' => 'Equipment', 'route' => 'admin.equipment.parseCsvImport'])
                    </div>
                </div>
            @endcan
            <div class="card">
                            ...
                            <thead>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <select class="search">
                                            <option value>{{ trans('global.all') }}</option>
                                            @foreach($equipment_types as $key => $item)
                            ...
                            <tbody>
                                @foreach($equipment as $key => $equipment)
                                    <tr data-entry-id="{{ $equipment->id }}">
                                        ...
                                        <td>
                                            @can('equipment_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.equipment.show', $equipment->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('equipment_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.equipment.edit', $equipment->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('equipment_delete')
                                                <form action="{{ route('frontend.equipment.destroy', $equipment->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('equipment_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.equipment.massDestroy') }}",
