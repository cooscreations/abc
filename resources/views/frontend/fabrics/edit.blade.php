    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.fabrics.update", [$fabric->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            <label class="required" for="name">{{ trans('cruds.fabric.fields.name') }}</label>
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $fabric->name) }}" required>
                            @if($errors->has('name'))
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="fabric_group_id" id="fabric_group_id" required>
                                @foreach($fabric_groups as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('fabric_group_id') ? old('fabric_group_id') : $fabric->fabric_group->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="fabric_code" id="fabric_code" value="{{ old('fabric_code', $fabric->fabric_code) }}">
