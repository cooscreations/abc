...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.productSku.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.product-skus.update", [$productSku->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="product_sku">{{ trans('cruds.productSku.fields.product_sku') }}</label>
                <input class="form-control {{ $errors->has('product_sku') ? 'is-invalid' : '' }}" type="text" name="product_sku" id="product_sku" value="{{ old('product_sku', $productSku->product_sku) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('product') ? 'is-invalid' : '' }}" name="product_id" id="product_id">
                    @foreach($products as $id => $entry)
                        <option value="{{ $id }}" {{ (old('product_id') ? old('product_id') : $productSku->product->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('prod_dev_stage') ? 'is-invalid' : '' }}" name="prod_dev_stage_id" id="prod_dev_stage_id">
                    @foreach($prod_dev_stages as $id => $entry)
                        <option value="{{ $id }}" {{ (old('prod_dev_stage_id') ? old('prod_dev_stage_id') : $productSku->prod_dev_stage->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('size') ? 'is-invalid' : '' }}" name="size_id" id="size_id" required>
                    @foreach($sizes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('size_id') ? old('size_id') : $productSku->size->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('size'))
                    <div class="invalid-feedback">
