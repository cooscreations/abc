...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.currency-rates.update", [$currencyRate->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="usd_value">{{ trans('cruds.currencyRate.fields.usd_value') }}</label>
                <input class="form-control {{ $errors->has('usd_value') ? 'is-invalid' : '' }}" type="number" name="usd_value" id="usd_value" value="{{ old('usd_value', $currencyRate->usd_value) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('valid_until') ? 'is-invalid' : '' }}" type="text" name="valid_until" id="valid_until" value="{{ old('valid_until', $currencyRate->valid_until) }}">
