...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-defaultProductGroupComponentParts">
                <thead>
                    <tr>
                        <th width="10">
                        ...
                        <tr data-entry-id="{{ $componentPart->id }}">
                            ...
                            <td>
                                @can('component_part_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.component-parts.show', $componentPart->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('component_part_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.component-parts.edit', $componentPart->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('component_part_delete')
                                    <form action="{{ route('admin.component-parts.destroy', $componentPart->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-defaultProductGroupComponentParts:not(.ajaxTable)').DataTable({ buttons: dtButtons })
