...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.fabrics.update", [$fabric->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.fabric.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $fabric->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('fabric_code') ? 'is-invalid' : '' }}" type="text" name="fabric_code" id="fabric_code" value="{{ old('fabric_code', $fabric->fabric_code) }}">
