...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-authorisedUserTypesDocuments">
                <thead>
                    <tr>
                        <th width="10">
                ...
                <tbody>
                    @foreach($documents as $key => $document)
                        <tr data-entry-id="{{ $document->id }}">
                            ...
                            <td>
                                @foreach($document->related_orders as $key => $item)
                                    <span class="badge badge-info">{{ $item->afa_order_num }}</span>
                                @endforeach
                            </td>
                            <td>
                                @can('document_show')
...
@endcan
  ...
  let table = $('.datatable-authorisedUserTypesDocuments:not(.ajaxTable)').DataTable({ buttons: dtButtons })
