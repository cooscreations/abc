@can('company_role_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.company-roles.create') }}">
...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-contactCompanyRoles">
                <thead>
                    <tr>
                        <th width="10">
                        ...
                        <tr data-entry-id="{{ $companyRole->id }}">
                            ...
                            <td>
                                @can('company_role_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.company-roles.show', $companyRole->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('company_role_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.company-roles.edit', $companyRole->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('company_role_delete')
                                    <form action="{{ route('admin.company-roles.destroy', $companyRole->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('company_role_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.company-roles.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-contactCompanyRoles:not(.ajaxTable)').DataTable({ buttons: dtButtons })
