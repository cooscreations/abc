...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.yes-no-maybes.update", [$yesNoMaybe->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="name">{{ trans('cruds.yesNoMaybe.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $yesNoMaybe->name) }}">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('notes') ? 'is-invalid' : '' }}" name="notes" id="notes">{{ old('notes', $yesNoMaybe->notes) }}</textarea>
                @if($errors->has('notes'))
                    <div class="invalid-feedback">
                        {{ $errors->first('notes') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('default_css_class') ? 'is-invalid' : '' }}" type="text" name="default_css_class" id="default_css_class" value="{{ old('default_css_class', $yesNoMaybe->default_css_class) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('icon') ? 'is-invalid' : '' }}" type="text" name="icon" id="icon" value="{{ old('icon', $yesNoMaybe->icon) }}">
                @if($errors->has('icon'))
                    <div class="invalid-feedback">
                        {{ $errors->first('icon') }}
                    </div>
