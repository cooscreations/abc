    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.prices.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('sku') ? 'is-invalid' : '' }}" name="sku_id" id="sku_id" required>
                    @foreach($skus as $id => $entry)
                        <option value="{{ $id }}" {{ old('sku_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('sku'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('default_raw_material') ? 'is-invalid' : '' }}" name="default_raw_material_id" id="default_raw_material_id">
                    @foreach($default_raw_materials as $id => $entry)
                        <option value="{{ $id }}" {{ old('default_raw_material_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('price_list') ? 'is-invalid' : '' }}" name="price_list_id" id="price_list_id">
                    @foreach($price_lists as $id => $entry)
                        <option value="{{ $id }}" {{ old('price_list_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
