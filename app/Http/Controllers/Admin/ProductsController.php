...
class ProductsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $query = Product::with(['primary_nickname', 'product_code_group', 'default_function', 'product_collection', 'default_group', 'default_storage', 'default_gas_lift_config', 'default_drawer_config', 'default_drawer_movement', 'default_tv_config', 'default_visitor_config', 'extra_letters_used_in_skus', 'primary_material', 'primary_suppliers', 'std_packaging'])->select(sprintf('%s.*', (new Product())->table));
            ...
            $table->editColumn('afa_model_number', function ($row) {
                return $row->afa_model_number ? $row->afa_model_number : '';
            });
            $table->addColumn('product_collection_name', function ($row) {
                return $row->product_collection ? $row->product_collection->name : '';
            });

            $table->addColumn('default_group_name', function ($row) {
                return $row->default_group ? $row->default_group->name : '';
            });

            $table->editColumn('primary_suppliers', function ($row) {
                $labels = [];
                foreach ($row->primary_suppliers as $primary_supplier) {
                    $labels[] = sprintf('<span class="label label-info label-many">%s</span>', $primary_supplier->company_short_code);
                }

                return implode(' ', $labels);
            });

            $table->rawColumns(['actions', 'placeholder', 'product_collection', 'default_group', 'primary_suppliers']);
        ...
        return view('admin.products.index', compact('product_nicknames', 'product_code_groups', 'product_functions', 'product_collections', 'product_groups', 'storage_options', 'gas_lift_configs', 'drawer_configs', 'drawer_movements', 'tv_bed_configs', 'visitor_bed_configs', 'product_sku_letters', 'raw_materials', 'contact_companies', 'packagings'));
    }
    ...
    public function create()
    {
        ...
        return view('admin.products.create', compact('primary_nicknames', 'product_code_groups', 'default_functions', 'product_collections', 'default_groups', 'default_storages', 'default_gas_lift_configs', 'default_drawer_configs', 'default_drawer_movements', 'default_tv_configs', 'default_visitor_configs', 'extra_letters_used_in_skus', 'primary_materials', 'primary_suppliers', 'std_packagings'));
    }
    ...
    public function edit(Product $product)
    {
        ...
        return view('admin.products.edit', compact('primary_nicknames', 'product_code_groups', 'default_functions', 'product_collections', 'default_groups', 'default_storages', 'default_gas_lift_configs', 'default_drawer_configs', 'default_drawer_movements', 'default_tv_configs', 'default_visitor_configs', 'extra_letters_used_in_skus', 'primary_materials', 'primary_suppliers', 'std_packagings', 'product'));
    }
